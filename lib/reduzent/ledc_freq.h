...
// HIGHEST resolution (finest duty granularity) that still fits the divider.
...
// Highest duty resolution (bits) at which `freq_hz` can be produced from
...
static inline uint8_t ledc_resolution_for(uint32_t src_clk_hz, uint32_t freq_hz) {
    ...
    for (uint8_t res = LEDC_RES_MAX; res >= LEDC_RES_MIN; res--) {
